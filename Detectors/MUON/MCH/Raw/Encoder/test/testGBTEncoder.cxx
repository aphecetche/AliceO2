...
template <>
std::vector<uint8_t> refBufferGBT<BareGBTEncoder>()
{
  return std::vector<uint8_t>(REF_BUFFER_GBT_BARE.begin(), REF_BUFFER_GBT_BARE.end());
}

template <>
std::vector<uint8_t> refBufferGBT<UserLogicGBTEncoder>()
{
...
BOOST_AUTO_TEST_CASE_TEMPLATE(EncodeABuffer, T, testTypes)
{
  auto buffer = createGBTBuffer<T>();
  size_t n = refBufferGBT<T>().size();
  BOOST_CHECK_GE(buffer.size(), n);
  BOOST_CHECK(std::equal(begin(buffer), end(buffer), begin(refBufferGBT<T>())));
}
