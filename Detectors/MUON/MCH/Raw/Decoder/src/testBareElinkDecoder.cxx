...
{
  BOOST_CHECK_THROW(BareElinkDecoder dec(0, 40, handlePacketPrint("dummy")), std::invalid_argument);
  BOOST_CHECK_NO_THROW(BareElinkDecoder dec(0, 39, handlePacketPrint("dummy")));
}
...
BOOST_AUTO_TEST_CASE(Decoding10)
{
  ...
  BareElinkDecoder e(0, 0, hp, false);
...
BOOST_AUTO_TEST_CASE(Decoding20)
{
  ...
  npackets = 0;
  BareElinkDecoder e2(0, 0, nullptr);
  for (int i = 0; i < enc.size() - 1; i += 2) {
