...
UserLogicGBTDecoder::UserLogicGBTDecoder(int cruId,
  ...
  : mCruId(cruId),
    mGbtId(gbtId),
    mDSDecoders{::makeArray<40>([=](size_t i) { return UserLogicDSDecoder(cruId, i, sampaChannelHandler, chargeSumMode); })},
    mNofGbtWordsSeens{0}
{
...
void UserLogicGBTDecoder::append(gsl::span<uint8_t> buffer)
{
  ...
  for (size_t i = 0; i < buffer.size(); i += 8) {
    ...
    assertIsInRange("dsid", dsid, 0, 39);
...
void UserLogicGBTDecoder::printStatus(int maxelink) const
{
}
