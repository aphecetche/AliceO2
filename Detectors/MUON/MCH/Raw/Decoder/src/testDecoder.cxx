...
BOOST_AUTO_TEST_CASE(Test0)
{
  ...
  createBareDecoder<RAWDataHeaderV4>(handleRDH, ch);
  createBareDecoder<RAWDataHeaderV4>(handleRDH, [](uint8_t cruId, uint8_t linkId, uint8_t chip, uint8_t channel, SampaCluster sc) {
  });
}
...
BOOST_AUTO_TEST_CASE(TestDecoding)
{
  ...
  auto decode = createBareDecoder<RAWDataHeaderV4>(handleRDH, handlePacketStoreAsVec(result));
  decode(testBuffer);
