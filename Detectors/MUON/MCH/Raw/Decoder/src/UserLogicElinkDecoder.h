...
void UserLogicElinkDecoder<CHARGESUM>::findSync()
{
  ...
  if (verbose)
    std::cout << fmt::format("[findSync] bitBuffer={:08X} mask={} checkpoint={}\n", mBitBuffer, mMask, mCheckpoint);
  if (mBitBuffer != sync) {
    mBitBuffer >>= 10;
    mMask -= 10;
    return;
  ...
  if (verbose)
    std::cout << fmt::format("[findSync] SYNC found\n");
...
void UserLogicElinkDecoder<CHARGESUM>::handleHeader()
{
  ...
  if (verbose)
    std::cout << fmt::format("[handleHeader] bitBuffer={:08X} mask={} checkpoint={}\n", mBitBuffer, mMask, mCheckpoint);
  ...
  switch (mSampaHeader.packetType()) {
    ...
    case SampaPacketType::Data:
      ...
      if (verbose)
        std::cout << fmt::format("[handleHeader] data header found, nof10BitsWordsToRead={}\n", mNof10BitsWordsToRead);
...
void UserLogicElinkDecoder<CHARGESUM>::handleReadSample()
{
  ...
  oneLess10BitWord();
  if (verbose)
    std::cout << fmt::format("[handleReadSample] mNofSamples={} mNof10BitsWordsToRead={} samples.size={}\n",
...
{
  if (verbose)
    std::cout << fmt::format("[process {}-{}] bitBuffer={:08X} state=", mDsId.elinkGroupId(), mDsId.elinkIndexInGroup(), mBitBuffer) << name(mState) << std::endl;
  switch (mState) {
    case State::LookingForSync:
      findSync();
      break;
