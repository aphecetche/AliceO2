...
uint64_t eightBytes(gsl::span<const uint8_t> buffer)
{
...
template <>
RAWDataHeaderV4 createRDH(gsl::span<const uint8_t> buffer)
{
  if (buffer.size() < 64) {
...
template <typename RDH>
int forEachRDH(gsl::span<const uint8_t> buffer, std::function<void(const RDH&, gsl::span<const uint8_t>::size_type)> f)
{
  assert(buffer.size() % 4 == 0);
  RDH rdh;
  int index{0};
...
template int showRDHs<RAWDataHeaderV4>(gsl::span<const uint8_t> buffer);
template int countRDHs<RAWDataHeaderV4>(gsl::span<const uint8_t> buffer);
template int forEachRDH(gsl::span<const uint8_t> buffer, std::function<void(const RAWDataHeaderV4&)> f);
template int forEachRDH(gsl::span<uint8_t> buffer, std::function<void(RAWDataHeaderV4&, gsl::span<uint8_t>::size_type)> f);
template int forEachRDH(gsl::span<const uint8_t> buffer, std::function<void(const RAWDataHeaderV4&, gsl::span<const uint8_t>::size_type)> f);
